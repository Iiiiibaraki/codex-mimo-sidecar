---
name: mimo-codex-subagent
description: Delegate self-contained side tasks to a MiMo-backed Codex CLI subagent through a clean-environment wrapper with persistent sessions and an interactive Terminal chat window. Use for independent exploration, code review, debugging, log inspection, implementation attempts, or benchmark supervision where MiMo can work from a clear brief.
---

# MiMo Codex Subagent (Community)
...
```bash
git clone <repo-url> ~/.codex/skills/mimo-codex-subagent
...
```bash
ln -s ~/.codex/skills/mimo-codex-subagent/scripts/codex-mimo-subagent \
  ~/.codex/bin/codex-mimo-subagent
```

## Prerequisites

A working MiMo profile in `~/.codex/config.toml`. Run `--configure` once to set it up interactively (detects VibeAround proxy on `127.0.0.1:12358` or instructs CC switch setup), then verifies connectivity with `codex doctor`.

```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --configure
```

## Workflow

Each execution opens a Terminal window that shows live task output, then becomes a `mimo>` follow-up prompt. Type `/exit` to leave the session idle for later resume.
...
```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --cd "$PROJECT" \
  "Explore relevant implementations. Report key files, conclusions, and next steps."
```

A session ID is recorded to `/tmp/mimo-subagent-sessions.txt` keyed by workdir.
...
```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --cd "$PROJECT" --status
```
...
```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --cd "$PROJECT" --list
```
...
```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --cd "$PROJECT" --resume \
...
```bash
"<path-to-skill>/scripts/codex-mimo-subagent" --session-id <UUID> \
...
| Flag | Effect |
| ---- | ------ |
| `--profile NAME` | Codex profile file name (default `mimo`, loaded from `$CODEX_HOME/mimo.config.toml`). |
...
```bash
API_KEY="$SOME_SECRET" \
  "<path-to-skill>/scripts/codex-mimo-subagent" \
...
Use the Terminal `mimo>` prompt only after the subagent's initial task completes — the session persists and the window remains interactive.
